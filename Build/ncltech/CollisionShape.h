...
{
	CollisionEdge(const nclgl::Maths::Vector3& a, const nclgl::Maths::Vector3& b)
		: _v0(a), _v1(b) {}
...
public:
	...
	virtual nclgl::Maths::Matrix3 BuildInverseInertia(float invMass) const = 0;
...
//<----- USED BY COLLISION DETECTION ----->
	...
	virtual void GetCollisionAxes(
		...
		std::vector<nclgl::Maths::Vector3>& out_axes) const = 0;

	//Returns closest point on the collision shape to the given point
	virtual nclgl::Maths::Vector3 GetClosestPoint(const nclgl::Maths::Vector3& point) const = 0;
	...
	virtual void GetMinMaxVertexOnAxis(
		...
		nclgl::Maths::Vector3& out_max) const = 0;
	...
	virtual void GetIncidentReferencePolygon(
		...
		std::vector<nclgl::Maths::Plane>& out_adjacent_planes) const = 0;
